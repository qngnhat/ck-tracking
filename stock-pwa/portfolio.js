...
window.__SSI_PORTFOLIO__ = (function () {
   ...
   *  - Setup score hiện tại (từ analysis.js)
   *  - Unrealized P&L %
   *  - Có trong DCA top picks không
   * Returns: {priority, icon, text, color}
   */
  function recommendAction(holding, analysis, inDcaTop) {
    ...
    // Critical sell signals
    if (score < -3) {
      return {
        priority: 1, icon: "🚨", color: "#ff4444",
        text: "Setup xấu — cân nhắc thoát vị thế",
      };
    }
    if (pnlPct < -8 && score < 0) {
      return {
        priority: 1, icon: "⚠️", color: "#ff5722",
        text: `Lỗ ${pnlPct.toFixed(1)}% + Setup yếu — review thesis hoặc cắt lỗ`,
      };
    }
    ...
    if (score >= 4 && inDcaTop) {
      return {
        ...
        text: "Setup tốt + còn trong DCA top — có thể tilt buy",
      };
    }
    ...
    if (score >= -3 && score < 0) {
      return {
        ...
        text: "Setup yếu — theo dõi vùng hỗ trợ",
      };
    }

    // Removed from DCA top
    if (!inDcaTop && holding.first_buy_date) {
      // Only flag for old holdings (held > 30 days)
    ...
    return {
      ...
      text: pnlPct > 0
        ? `Giữ. Lãi nhẹ ${pnlPct.toFixed(1)}%`
        : pnlPct < 0
        ? `Giữ. Lỗ nhẹ ${pnlPct.toFixed(1)}%`
        : "Giữ. Trung tính",
    };
  }
